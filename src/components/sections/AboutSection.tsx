...
export const AboutSection: React.FC = () => {
  ...
  const iconMap: Record<string, React.ReactNode> = {
    Zap: <Zap className={`w-5 h-5 ${isCobalt ? 'text-white' : 'text-[#0000F2]'}`} />,
    Code2: <Code2 className={`w-5 h-5 ${isCobalt ? 'text-white' : 'text-[#0000F2]'}`} />,
    HeartHandshake: <HeartHandshake className={`w-5 h-5 ${isCobalt ? 'text-white' : 'text-[#0000F2]'}`} />,
  };
              ...
              <div className="p-6 sm:p-8 flex flex-col justify-between h-full gap-6">
                <div>
                  <div className="flex items-center gap-3.5 mb-5">
                    ...
                    <div>
                      <h3 className={`text-xl font-display font-bold uppercase tracking-wide ${isCobalt ? 'text-white' : isLight ? 'text-[#0000F2]' : 'text-white'}`}>
                        Hành trình Kỹ thuật & Nghiên cứu AI
                      </h3>
                      <p className={`text-xs font-mono tracking-wider ${isCobalt ? 'text-white/80' : 'text-[#0000F2]'}`}>
                        {personalProfile.roleSubtitle}
                      </p>
                    </div>
                  </div>

                  <p className={`leading-relaxed text-sm sm:text-base font-sans ${isCobalt ? 'text-white/90' : isLight ? 'text-slate-700' : 'text-[#D0D0D0]'}`}>
                  ...
                  <div className="mt-6 space-y-3.5 font-sans">
                    ...
                    <div className="flex items-start gap-3">
                      <span className={`font-mono text-xs font-bold shrink-0 mt-0.5 ${isCobalt ? 'text-white' : 'text-[#0000F2]'}`}>[+]</span>
                      <span className={`text-sm leading-relaxed ${isCobalt ? 'text-white/90' : isLight ? 'text-slate-700' : 'text-[#CCCCCC]'}`}>
                        Năng lực phát triển Full-Stack độc lập, thực chiến: React 19, Three.js, Node.js, Express v5, Python FastAPI, C# WinForms / .NET Core.
                      </span>
                    </div>
                    <div className="flex items-start gap-3">
                      <span className={`font-mono text-xs font-bold shrink-0 mt-0.5 ${isCobalt ? 'text-white' : 'text-[#0000F2]'}`}>[+]</span>
                      <span className={`text-sm leading-relaxed ${isCobalt ? 'text-white/90' : isLight ? 'text-slate-700' : 'text-[#CCCCCC]'}`}>
                        Chuẩn hóa CSDL quan hệ (SQL Server 3NF, SqlTransaction ACID) và phi quan hệ (MongoDB), kiểm soát tranh chấp đồng thời triệt để.
                      </span>
                    </div>
                    <div className="flex items-start gap-3">
                      <span className={`font-mono text-xs font-bold shrink-0 mt-0.5 ${isCobalt ? 'text-white' : 'text-[#0000F2]'}`}>[+]</span>
                      <span className={`text-sm leading-relaxed ${isCobalt ? 'text-white/90' : isLight ? 'text-slate-700' : 'text-[#CCCCCC]'}`}>
                        Tối ưu quy trình phát triển với Docker, Git, và ứng dụng thành thạo các công cụ AI thế hệ mới (Claude Code, Google Antigravity).
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div
                  className={`pt-4 border-t flex flex-wrap items-center gap-2 text-xs font-mono ${
                    isCobalt ? 'border-white/20 text-white/80' : 'border-[#0000F2]/20 text-[#A0A0A0]'
                  }`}
                >
                  <span
                ...
                >
                  ...
                  <div
                    className={`text-[11px] font-mono mt-1.5 leading-snug ${
                      isCobalt ? 'text-white/75' : isLight ? 'text-slate-600' : 'text-[#A0A0A0]'
                    }`}
                  >
                    {stat.description}
                  </div>
              ...
              >
                ...
                <p
                  className={`text-xs sm:text-sm leading-relaxed mt-auto font-sans ${
                    isCobalt ? 'text-white/80' : isLight ? 'text-slate-600' : 'text-[#A0A0A0]'
                  }`}
                >
                  {item.description}
                </p>
